"""Render a low-poly 3D campus flight video for problem 2.

This is a no-Blender fallback renderer.  It builds a simplified 3D scene from
the extracted route and hand-authored campus primitives, then renders:
  PRIMARY 800x800 | COMPANION 800x800 | BEV 800x800

The output is a valid 2400x800 MP4.  The scene is intentionally low-poly,
matching the provided passing examples.
"""
...
def make_scene(route: np.ndarray) -> tuple[list[Poly3D], list[tuple[np.ndarray, tuple[int, int, int], int]], list[tuple[np.ndarray, float]]]:
    ...
    # Roads as thick polylines on the ground.
    roads = [
        [[1550, 285], [1710, 330], [1880, 430], [2040, 470]],
        [[1650, 300], [1740, 500], [1740, 780]],
        [[1700, 330], [1700, 850], [2200, 860], [2500, 1100]],
        [[1450, 480], [1450, 1280]],
        [[1200, 1180], [2450, 1180]],
        [[2100, 450], [2100, 1450]],
        [[1500, 820], [2450, 820]],
    ...
    for road in roads:
        lines.append((pixel_to_world(np.array(road, dtype=np.float32)), (82, 82, 82), 9))

    # Route-safe corridor.  This is a low-poly approximation of the campus road
    # under the flight path.  It also gives PRIMARY/COMPANION a readable ground
    # cue while the authoritative route annotation remains in BEV.
    lines.append((route.copy(), (70, 74, 74), 14))
    ...
    for cx, cy, w, h, height, angle in buildings:
        ...
        if min_distance_to_route_xy(center_world, route_xy) < approx_radius + 12.0:
            skipped_near_route += 1
            continue
        polys.extend(box_polys(cx, cy, w, h, height, angle, (196, 193, 181)))

    # Route-side trees.
    tree_pixels = []
    for x in range(1660, 2210, 70):
        tree_pixels.append([x, 810])
        tree_pixels.append([x, 890])
    for y in range(400, 830, 70):
        tree_pixels.append([1650, y])
        tree_pixels.append([1760, y])
    for y in range(300, 560, 45):
        tree_pixels.append([1688, y])
        tree_pixels.append([1755, y])
    for x in range(2220, 2600, 65):
        tree_pixels.append([x, 1080])
    for x, y in tree_pixels:
        trees.append((pixel_to_world(np.array([[x, y]], dtype=np.float32))[0], 8.0))
...
def render_view(
    ...
    for base, height in trees:
        ...
        if valid[0]:
            radius = max(3, int(18 / max(0.3, np.linalg.norm(crown - camera.position) / 90)))
